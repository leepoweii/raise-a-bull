...
- **Per-call token logging** — `ClaudeRunner.run()` accepts an optional `source: str` parameter and logs `LLM call: source=... input=N output=N total=N` at INFO level after every completed LLM call. `source` defaults to blank unless a caller supplies it. Logger: `raisebull.runner`
