...
重新編譯 `IDENTITY.md`，將 `identity/managed/` 和 `identity/local/` 的內容合併為 raise-a-bull 讀取的單一檔案。
...
- **local knowledge**（geography, transport, climate）→ 由 `feed.sh` 維護在 `identity/managed/`，永遠不要手動修改
...
| 在地通用（系統維護） | `identity/managed/`（不要動） | geography, transport, climate |
